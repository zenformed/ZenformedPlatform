...
import { reactivateSubscriptionOnCore } from '@/infrastructure/coreApi/subscriptionReactivateClient';
...
export async function POST(request: NextRequest): Promise<NextResponse> {
  if (!runtimeModes.isSaasMode() || runtimeModes.useMockAuth()) {
    return NextResponse.json(
      {
        error: 'bad_request',
        message: 'Subscription reactivation requires SaaS mode with real Supabase auth.',
      },
      { status: 400 }
    );
  }
  ...
  const result = await reactivateSubscriptionOnCore(raw, productSlug);
  if (!result.ok) {
    if (result.error.kind === 'http_error') {
      ...
      return NextResponse.json(result.error.body ?? { error: 'reactivate_failed' }, {
