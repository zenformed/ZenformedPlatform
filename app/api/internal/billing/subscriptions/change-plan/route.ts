...
import { changeSubscriptionPlanOnCore } from '@/infrastructure/coreApi/subscriptionChangePlanClient';
...
function readProrationDate(body: unknown): string | null {
  if (body == null || typeof body !== 'object') return null;
  const raw = (body as Record<string, unknown>).prorationDate;
  if (typeof raw !== 'string' || raw.trim() === '') return null;
...
export async function POST(request: NextRequest): Promise<NextResponse> {
  ...
  if (!runtimeModes.isSaasMode() || runtimeModes.useMockAuth()) {
    ...
    return NextResponse.json(

      {
        ...
        message: 'Subscription plan changes require SaaS mode with real Supabase auth.',
  ...
  const prorationDate = readProrationDate(body);
  ...
  const result = await changeSubscriptionPlanOnCore(raw, {
    productSlug,
    planSlug,
    billingCycle,
  ...
  if (!result.ok) {
    ...
    if (result.error.kind === 'http_error') {
      ...
      return NextResponse.json(result.error.body ?? { error: 'change_plan_failed' }, {
