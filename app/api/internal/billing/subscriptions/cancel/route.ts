...
import { cancelSubscriptionOnCore } from '@/infrastructure/coreApi/subscriptionCancelClient';
...
export async function POST(request: NextRequest): Promise<NextResponse> {
  if (!runtimeModes.isSaasMode() || runtimeModes.useMockAuth()) {
    return NextResponse.json(
      {
        error: 'bad_request',
        message: 'Subscription cancellation requires SaaS mode with real Supabase auth.',
      },
      { status: 400 }
    );
  }
  ...
  const result = await cancelSubscriptionOnCore(raw, productSlug);
  if (!result.ok) {
    if (result.error.kind === 'http_error') {
      ...
      return NextResponse.json(result.error.body ?? { error: 'cancel_failed' }, {
