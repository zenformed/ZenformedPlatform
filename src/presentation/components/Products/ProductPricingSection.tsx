...
export type ProductPricingSectionProps = {
  ...
  readonly suppressSectionWrapper?: boolean;
};
...
export function ProductPricingSection({
  ...
  variant = 'standalone',
  sectionId,
  suppressSectionWrapper = false,
}: ProductPricingSectionProps): ReactElement {
  ...
  const getAccessToken = useCallback((): string | null => sessionRef.current?.access_token ?? null, []);
  ...
  const pricingClassName = `${styles.pricingSection} ${embedded ? styles.pricingSectionEmbedded : ''} ${singlePlan ? styles.pricingSectionSingle : ''}`;
    ...
    <>
      <div className={styles.pricingHeader}>
          {!embedded ? (
            ...
            <>
              <h2 className={styles.salesSectionTitle}>
                {config.title}{' '}
                {config.titleHighlight != null && config.titleHighlight.trim() !== '' ? (
                  <span className={styles.pricingTitleGradient}>{config.titleHighlight}</span>
                ) : null}
              </h2>
              <p className={styles.salesSectionIntro}>
                Start with a 14-day free trial or choose a paid plan. Upgrade, downgrade, or cancel
                anytime.
              </p>
            </>
          )}
  ...
  return (
    <>
      {suppressSectionWrapper ? (
        <div className={pricingClassName}>{pricingContent}</div>
      ) : (
        <section id={sectionId} className={pricingClassName}>
          {pricingContent}
        </section>
      )}
