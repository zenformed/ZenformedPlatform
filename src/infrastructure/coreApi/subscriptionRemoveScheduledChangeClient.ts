...
export type RemoveScheduledChangeResponse = {
  productSlug: string;
  entitlement: unknown;
};
...
function parseRemoveScheduledChangeJson(json: unknown): RemoveScheduledChangeResponse | null {
...
/** POST /billing/subscriptions/:productSlug/remove-scheduled-change — cancel a scheduled downgrade. */
export async function removeScheduledPlanChangeOnCore(
  accessToken: string,
  productSlug: string
): Promise<CoreApiResult<RemoveScheduledChangeResponse>> {
  ...
  const url = `${normalizeBaseUrl(base)}/billing/subscriptions/${encodeURIComponent(slug)}/remove-scheduled-change`;
  ...
  try {
    ...
    const parsed = parseRemoveScheduledChangeJson(json);
    if (parsed == null) {
      return { ok: false, error: { kind: 'invalid_payload' } };
    }
