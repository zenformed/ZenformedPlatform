...
function parseChangePlanJson(json: unknown): ChangeSubscriptionPlanResponse | null {
...
/** POST /billing/subscriptions/:productSlug/change-plan — upgrade/downgrade existing subscription. */

export async function changeSubscriptionPlanOnCore(
  ...
  input: {
    ...
    readonly prorationDate?: string;
  }

): Promise<CoreApiResult<ChangeSubscriptionPlanResponse>> {
  ...
  const url = `${normalizeBaseUrl(base)}/billing/subscriptions/${encodeURIComponent(slug)}/change-plan`;
  ...
  try {
    ...
    const res = await fetch(url, {
      ...
      body: JSON.stringify({
        ...
        ...(input.prorationDate != null ? { prorationDate: input.prorationDate } : {}),
      }),
    ...
    const parsed = parseChangePlanJson(json);
