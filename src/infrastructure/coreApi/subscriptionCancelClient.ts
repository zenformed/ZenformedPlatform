...
function parseCancelSubscriptionJson(json: unknown): CancelSubscriptionResponse | null {
...
/** POST /billing/subscriptions/:productSlug/cancel — schedule cancel at period end. */
export async function cancelSubscriptionOnCore(
  accessToken: string,
  productSlug: string
): Promise<CoreApiResult<CancelSubscriptionResponse>> {
  ...
  const url = `${normalizeBaseUrl(base)}/billing/subscriptions/${encodeURIComponent(slug)}/cancel`;
  ...
  try {
    ...
    const parsed = parseCancelSubscriptionJson(json);
    if (parsed == null) {
      return { ok: false, error: { kind: 'invalid_payload' } };
    }
